local function __GanKunZHub_Init_Main__()
...
if not ModernV2 then
    ...
    if loaderOk then
        ...
        warn("[GanKunZ Hub] Vercel mirror failed, trying GitHub fallback:", loaderResult)
        ...
        if fallbackOk then
            ...
            warn("[GanKunZ Hub] Failed to load ModernV2 from all sources:", fallbackResult)
        end
    end
end
...
local UIConfigPath = "GanKunZHub_UI_Theme.json"
local GanKunZUI_Solid = false
local GanKunZUI_Color = "Blue"

pcall(function()
    if isfile and readfile and isfile(UIConfigPath) then
        local data = HttpService:JSONDecode(readfile(UIConfigPath))
        GanKunZUI_Solid = data.Solid
        GanKunZUI_Color = data.Color or "Blue"
    end
end)

if GanKunZUI_Color == "Default" then
    GanKunZUI_Color = "Blue"
end
...
if ModernV2 then
    ...
    Window = ModernV2:Window({
        ...
        Config = {
            ConfigFolder = "GanKunZHubViolenceDistrict",
            AutoSaveFile = "GANKZ_VD",
            AutoSave = false,
            AutoLoad = false,
            Overwrite = true,
            Format = "JSON",
...
function VD_UpdateCrosshair()
    ...
    CrosshairGui.Name = "GANKZ_Crosshair"
...
function GetSafeChamsFolder()
    ...
    local f = pg:FindFirstChild("GANKZ_WorkspaceChams")
    if not f then
        f = Instance.new("Folder")
        f.Name = "GANKZ_WorkspaceChams"
        f.Parent = pg
    end
    VD_ChamsFolder = f
    return f
...
local ConfigFolderName = "GanKunZHub_VD"
local HttpService = game:GetService("HttpService")
...
function GANKZ_SaveConfig(name)
...
local VD_To_Flag = {
    ...
    VIS_GanKunZKiller = "GanKunZ Killer Display",
...
function GANKZ_LoadConfig(name)
    name = (name and name ~= "") and name or getgenv().CurrentConfigName
    if not name or name == "" then name = "Default" end
    local path = ConfigFolderName .. "/" .. name .. ".json"
    pcall(function()
        if readfile and isfile and isfile(path) then
            ...
            if getgenv().GANKZ_SyncLoadedFeatures then pcall(getgenv().GANKZ_SyncLoadedFeatures) end
        end
    end)
end

function GANKZ_DeleteConfig(name)
    name = (name and name ~= "") and name or getgenv().CurrentConfigName
...
function VD_ApplyWeather(themeName)
    local theme = GANKZ_WeatherPresets[themeName]
    if not theme then theme = GANKZ_WeatherPresets["Default"] end
...
-- CHARACTER REFS
-- =====================================================
-- Character, Humanoid, and Root are declared at the top of __GanKunZHub_Init_Main__
...
-- =====================================================
local GANKZ_WorldReg
getgenv().GANKZ_oldNamecall = nil

function setupAntiFail()
    if getgenv().GANKZ_AntiFailHooked then return end
    getgenv().GANKZ_AntiFailHooked = true
    task.spawn(function()
...
-- → infinite recursion → stack overflow → force close.
            ...
            _genv.GANKZ_oldNamecall = hookmetamethod(game, "__namecall", function(self, ...)
                ...
                if VD.KILLER_SilentAimFlask and method == "FireServer" then
                    ...
                    if ok and name == "ThrowFlask" then
                        ...
                        if closest then
                            ...
                            return _genv.GANKZ_oldNamecall(self, unpack(args))
                        end
                    end
                end

                if method == "FireServer" and not checkcaller() then
                    local flashRemote = _genv.GANKZ_FlashlightActivateRemote
                    if flashRemote and self == flashRemote and _genv.GANKZ_SetFlashlightAimActive then
                        local args = { ... }
                        pcall(_genv.GANKZ_SetFlashlightAimActive, args[2] == true, args[1])
                    end
                end

                if _genv.GANKZ_oldNamecall then
                    return _genv.GANKZ_oldNamecall(self, ...)
                end
            end)
...
-- =====================================================
getgenv().GANKZ_fpWasSet = false
getgenv().GANKZ_fpOriginal = nil

function RestoreFirstPersonCamera()
    if not getgenv().GANKZ_fpWasSet then return end
    getgenv().GANKZ_fpWasSet = false

    pcall(function()
        if getgenv().GANKZ_fpOriginal then
            LocalPlayer.CameraMode = getgenv().GANKZ_fpOriginal.CameraMode or Enum.CameraMode.Classic
            LocalPlayer.CameraMaxZoomDistance = getgenv().GANKZ_fpOriginal.CameraMaxZoomDistance or 128
            LocalPlayer.CameraMinZoomDistance = getgenv().GANKZ_fpOriginal.CameraMinZoomDistance or 0.5
        else
...
RunService.RenderStepped:Connect(function()
    pcall(function()
        if VD.SURV_FirstPerson then
            ...
            if isSurvivor then
                if not getgenv().GANKZ_fpWasSet then
                    getgenv().GANKZ_fpOriginal = {
                ...
                getgenv().GANKZ_fpWasSet = true
            elseif getgenv().GANKZ_fpWasSet then
...
-- Kontrol ditambahkan ke tab Visual lewat getgenv().GANKZ_AddVisualESPControls.
-- =====================================================
do
    if getgenv().GANKZ_VD_VisualESP_Cleanup then
        pcall(getgenv().GANKZ_VD_VisualESP_Cleanup)
    end

    local LP = LocalPlayer
    local GANKZ_Dead = false
    local GANKZ_ControlsAdded = false

    local GANKZ_ESPState = {
    ...
    getgenv().GANKZ_VD_VisualESP_State = GANKZ_ESPState
    ...
    local GANKZ_MapAdd, GANKZ_MapRem = {}, {}
    local GANKZ_PlayerConns = {}
    local GANKZ_Connections = {}
    local GANKZ_PalletState = setmetatable({}, { __mode = "k" })
    local GANKZ_WindowState = setmetatable({}, { __mode = "k" })
    local GANKZ_InstanceIds = setmetatable({}, { __mode = "k" })
    local GANKZ_GanKunZId = 0
    local GANKZ_PlayerLoopThread = nil
    local GANKZ_WorldLoopThread = nil
    local GANKZ_ESPFolder = nil

    local GANKZ_DisplayNames = {
        ["Motion Tracker"] = true,
        ["Gate"] = true,
        ["Flashlight"] = true,
        ["Bandage"] = true,
    ...
    local function GANKZ_Alive(inst)
        if not inst then return false end
        local ok, parent = pcall(function() return inst.Parent end)
        return ok and parent ~= nil
    end

    local function GANKZ_Clamp(n, lo, hi)
        n = tonumber(n) or lo
    ...
    local function GANKZ_PlayerKey(player)
    ...
    local function GANKZ_EspId(inst)
        if not inst then return "nil" end
        local id = GANKZ_InstanceIds[inst]
        if id then return id end
        GANKZ_GanKunZId = GANKZ_GanKunZId + 1
        id = tostring(GANKZ_GanKunZId)
        GANKZ_InstanceIds[inst] = id
        return id
    end

    local function GANKZ_GetESPParent()
    ...
    local function GANKZ_GetESPFolder()
        if GANKZ_ESPFolder and GANKZ_ESPFolder.Parent then
            return GANKZ_ESPFolder
        end

        local parent = GANKZ_GetESPParent()
        local old = parent:FindFirstChild("GanKunZHub_VisualESP") or parent:FindFirstChild("ZiaanHub_ESP")
        if old then old:Destroy() end

        local folder = Instance.new("Folder")
        folder.Name = "GanKunZHub_VisualESP"
        folder.Parent = parent
        GANKZ_ESPFolder = folder
        return folder
    end

    local function GANKZ_ClearPrefix(prefix, keepName)
        local folder = GANKZ_GetESPFolder()
    ...
    local function GANKZ_SafeNotify(title, content, duration)
        pcall(function()
    ...
    local function GANKZ_ValidPart(part)
        return part and GANKZ_Alive(part) and part:IsA("BasePart")
    end

    local function GANKZ_FirstBasePart(inst)
        if not GANKZ_Alive(inst) then return nil end
        if inst:IsA("BasePart") then return inst end
        if inst:IsA("Model") then
            if inst.PrimaryPart and inst.PrimaryPart:IsA("BasePart") and GANKZ_Alive(inst.PrimaryPart) then
                return inst.PrimaryPart
            end
            local part = inst:FindFirstChildWhichIsA("BasePart", true)
            if GANKZ_ValidPart(part) then return part end
        end
        if inst:IsA("Tool") then
            local handle = inst:FindFirstChild("Handle") or inst:FindFirstChildWhichIsA("BasePart")
            if GANKZ_ValidPart(handle) then return handle end
        end
        return nil
    end

    local function GANKZ_GetRole(player)
    ...
    local function GANKZ_PlayerRoleEnabled(player)
        local role = GANKZ_GetRole(player)
        if role == "Killer" then return GANKZ_ESPState.KillerESP end
        if role == "Spectator" then return GANKZ_ESPState.SpectatorESP end
        return GANKZ_ESPState.SurvivorESP
    end

    local function GANKZ_PlayerColor(player)
        local role = GANKZ_GetRole(player)
        if role == "Killer" then return GANKZ_ESPState.KillerColor end
        if role == "Spectator" then return GANKZ_ESPState.SpectatorColor end
        return GANKZ_ESPState.SurvivorColor
    end

    getgenv().GANKZ_VD_VisualESP_HasPlayerText = function(player)
        if not player or player == LP then return false end
        return GANKZ_ESPState.PlayerMasterESP
            and GANKZ_PlayerRoleEnabled(player)
            and (GANKZ_ESPState.Nametags or GANKZ_ESPState.DistanceESP)
    end

    local function GANKZ_EnsureHighlight(name, adornee, color, isPlayer)
        if not (adornee and GANKZ_Alive(adornee)) then return nil end
        local folder = GANKZ_GetESPFolder()
        GANKZ_ClearPrefix(name, name)
        ...
        if isPlayer then
            hl.FillTransparency = GANKZ_ESPState.ESPFillTransparency
            hl.OutlineTransparency = GANKZ_ESPState.ESPOutlineTransparency
        else
    ...
    local function GANKZ_DestroyChild(name)
        local folder = GANKZ_GetESPFolder()
        local child = folder:FindFirstChild(name)
        if child then child:Destroy() end
    end

    local function GANKZ_ClearPlayerESP(player)
        if not player or player == LP then return end
        local key = GANKZ_PlayerKey(player)
        GANKZ_DestroyChild("GANKZ_PlayerHL_" .. key)
        GANKZ_DestroyChild("GANKZ_PlayerTag_" .. key)
        GANKZ_DestroyChild("GANKZ_PlayerItem_" .. key)
    end

    local function GANKZ_ClearAllPlayerESP()
        for _, player in ipairs(Players:GetPlayers()) do
            if player ~= LP then
                GANKZ_ClearPlayerESP(player)
            end
        end
    end

    local function GANKZ_GetSurvivorItem(player)
        local character = player.Character
        if not character then return nil end
        for _, obj in ipairs(character:GetDescendants()) do
            if obj:IsA("Tool") or obj:IsA("Accessory") or obj:IsA("Model") then
                if GANKZ_DisplayNames[obj.Name] then
                    return obj.Name
                end
            end
        end
    ...
    local function GANKZ_GetItemImageId(itemName)
    ...
    local function GANKZ_SetBillboardLine(parent, index, count, data)
        ...
        label.TextSize = GANKZ_ESPState.ESPTextSize
        label.TextColor3 = data.Color
        label.Text = data.Text
    end

    local function GANKZ_PruneBillboardLines(parent, count)
    ...
    local function GANKZ_UpdatePlayerTag(player, character, head, color)
        local key = GANKZ_PlayerKey(player)
        local tagName = "GANKZ_PlayerTag_" .. key
        local folder = GANKZ_GetESPFolder()
        GANKZ_ClearPrefix("GANKZ_PlayerTag_" .. key, tagName)

        if not GANKZ_ValidPart(head) then
            GANKZ_DestroyChild(tagName)
            return
        end
        ...
        local distanceText = ""
        if GANKZ_ESPState.DistanceESP and root and targetRoot then
            distanceText = "[" .. tostring(math.floor((root.Position - targetRoot.Position).Magnitude)) .. "m]"
        end

        local nameText = GANKZ_ESPState.Nametags and player.Name or ""
        local mainLine = ""
        ...
        for i, data in ipairs(lines) do
            GANKZ_SetBillboardLine(tag, i, #lines, data)
        end
        GANKZ_PruneBillboardLines(tag, #lines)
    end

    local function GANKZ_UpdatePlayerItemIcon(player, torso)
        local key = GANKZ_PlayerKey(player)
        local iconName = "GANKZ_PlayerItem_" .. key
        local folder = GANKZ_GetESPFolder()
        GANKZ_ClearPrefix("GANKZ_PlayerItem_" .. key, iconName)

        if not GANKZ_ValidPart(torso) then
            GANKZ_DestroyChild(iconName)
            return
        end

        local itemName = GANKZ_GetSurvivorItem(player)
        local imageId = itemName and GANKZ_GetItemImageId(itemName) or nil
        if not imageId then
            GANKZ_DestroyChild(iconName)
            return
        end
    ...
    local GANKZ_ApplyPlayerESP
    GANKZ_ApplyPlayerESP = function(player)
        if GANKZ_Dead or not player or player == LP then return end
        local character = player.Character
        if not (character and GANKZ_Alive(character)) then
            GANKZ_ClearPlayerESP(player)
            return
        end

        local key = GANKZ_PlayerKey(player)
        local enabled = GANKZ_ESPState.PlayerMasterESP and GANKZ_PlayerRoleEnabled(player)
        if not enabled then
            GANKZ_ClearPlayerESP(player)
            return
        end

        local color = GANKZ_PlayerColor(player)
        ...
        GANKZ_EnsureHighlight("GANKZ_PlayerHL_" .. key, character, color, true)
        GANKZ_UpdatePlayerTag(player, character, head, color)

        if GANKZ_GetRole(player) == "Survivor" and GANKZ_ESPState.SurvivorItemsESP then
            GANKZ_UpdatePlayerItemIcon(player, torso)
        else
            GANKZ_DestroyChild("GANKZ_PlayerItem_" .. key)
        end
    end

    local function GANKZ_RefreshAllPlayers()
        for _, player in ipairs(Players:GetPlayers()) do
            if player ~= LP then
                pcall(GANKZ_ApplyPlayerESP, player)
            end
        end
    end

    local function GANKZ_StartPlayerLoop()
        if GANKZ_PlayerLoopThread then return end
        GANKZ_PlayerLoopThread = task.spawn(function()
            while not GANKZ_Dead and GANKZ_ESPState.PlayerMasterESP do
                GANKZ_RefreshAllPlayers()
                task.wait(0.25)
            end
            GANKZ_PlayerLoopThread = nil
        end)
    end

    local function GANKZ_WatchPlayer(player)
        if player == LP then return end
        if GANKZ_PlayerConns[player] then
            for _, conn in ipairs(GANKZ_PlayerConns[player]) do
                if conn then pcall(function() conn:Disconnect() end) end
            end
        end

        GANKZ_PlayerConns[player] = {}
        table.insert(GANKZ_PlayerConns[player], player.CharacterAdded:Connect(function(char)
            GANKZ_ClearPlayerESP(player)
            task.delay(0.15, function()
                if not GANKZ_Dead then pcall(GANKZ_ApplyPlayerESP, player) end
            end)
        end))
        table.insert(GANKZ_PlayerConns[player], player.CharacterRemoving:Connect(function()
            GANKZ_ClearPlayerESP(player)
        end))
        table.insert(GANKZ_PlayerConns[player], player:GetPropertyChangedSignal("Team"):Connect(function()
            GANKZ_ClearPlayerESP(player)
            pcall(GANKZ_ApplyPlayerESP, player)
        end))
    ...
    local function GANKZ_UnwatchPlayer(player)
        GANKZ_ClearPlayerESP(player)
        if GANKZ_PlayerConns[player] then
            for _, conn in ipairs(GANKZ_PlayerConns[player]) do
                if conn then pcall(function() conn:Disconnect() end) end
        ...
        GANKZ_PlayerConns[player] = nil
    end

    local function GANKZ_PickWorldPart(model, cat)
        if not (model and GANKZ_Alive(model)) then return nil end
        if cat == "Generator" then
            local hitbox = model:FindFirstChild("HitBox", true) or model:FindFirstChild("GeneratorPoint", true)
            if GANKZ_ValidPart(hitbox) then return hitbox end
        ...
        elseif cat == "Window" then
            ...
            if GANKZ_ValidPart(vault) then return vault end
        elseif cat == "SCPZombie" then
            local root = model:FindFirstChild("HumanoidRootPart", true)
            if GANKZ_ValidPart(root) then return root end
            local torso = model:FindFirstChild("UpperTorso", true) or model:FindFirstChild("Torso", true)
            if GANKZ_ValidPart(torso) then return torso end
            return nil
        end
        return GANKZ_FirstBasePart(model)
    end

    local function GANKZ_GeneratorLabel(model)
        local pct = tonumber(model:GetAttribute("RepairProgress")) or 0
        if pct >= 0 and pct <= 1.001 then pct = pct * 100 end
        pct = GANKZ_Clamp(pct, 0, 100)
        ...
        local hue = GANKZ_Clamp((pct / 100) * 0.33, 0, 0.33)
        return table.concat(parts, " "), Color3.fromHSV(hue, 1, 1)
    end

    local function GANKZ_HasBasePart(model)
        if not (model and GANKZ_Alive(model)) then return false end
        return model:FindFirstChildWhichIsA("BasePart", true) ~= nil
    end

    local function GANKZ_IsPalletGone(model)
        if not GANKZ_Alive(model) then return true end
        if not model:IsDescendantOf(Workspace) then return true end
        if GANKZ_PalletState[model] == "DEST" then return true end
        local ok, destroyed = pcall(function() return model:GetAttribute("Destroyed") end)
        if ok and destroyed == true then return true end
        return not GANKZ_HasBasePart(model)
    end

    local function GANKZ_WorldKey(cat, model)
        return "GANKZ_World_" .. cat .. "_" .. GANKZ_EspId(model)
    end

    local function GANKZ_ClearWorldVisual(cat, model)
        if not model then return end
        GANKZ_DestroyChild(GANKZ_WorldKey(cat, model) .. "_HL")
        GANKZ_DestroyChild(GANKZ_WorldKey(cat, model) .. "_Tag")
    end

    local function GANKZ_RemoveWorldEntry(cat, model)
        if not GANKZ_WorldReg[cat] or not GANKZ_WorldReg[cat][model] then return end
        GANKZ_ClearWorldVisual(cat, model)
        GANKZ_WorldReg[cat][model] = nil
    end

    local function GANKZ_EnsureWorldEntry(cat, model)
        if not GANKZ_Alive(model) or not GANKZ_WorldReg[cat] or GANKZ_WorldReg[cat][model] then return end
        if cat == "Palletwrong" and GANKZ_IsPalletGone(model) then return end
        local part = GANKZ_PickWorldPart(model, cat)
        if not GANKZ_ValidPart(part) then return end
        GANKZ_WorldReg[cat][model] = { part = part }
    end

    local function GANKZ_RegisterWorldDescendant(obj)
        if not GANKZ_Alive(obj) then return end
        local validCats = { Generator = true, Hook = true, Gate = true, Window = true, Palletwrong = true }

        if obj:IsA("Model") then
            if validCats[obj.Name] then
                GANKZ_EnsureWorldEntry(obj.Name, obj)
                return
            end
            local lower = obj.Name:lower()
            if lower:find("scp") or lower:find("zombie") then
                GANKZ_EnsureWorldEntry("SCPZombie", obj)
            end
            return
        end
            ...
            while parent and parent ~= Workspace do
                if parent:IsA("Model") then
                    if validCats[parent.Name] then
                        GANKZ_EnsureWorldEntry(parent.Name, parent)
                        return
                    end
                    local lower = parent.Name:lower()
                    if lower:find("scp") or lower:find("zombie") then
                        GANKZ_EnsureWorldEntry("SCPZombie", parent)
                        return
                    end
                end
    ...
    local function GANKZ_UnregisterWorldDescendant(obj)
        if not obj then return end
        local validCats = { Generator = true, Hook = true, Gate = true, Window = true, Palletwrong = true }

        if obj:IsA("Model") then
            if validCats[obj.Name] then
                GANKZ_RemoveWorldEntry(obj.Name, obj)
                return
            end
            local lower = obj.Name:lower()
            if lower:find("scp") or lower:find("zombie") then
                GANKZ_RemoveWorldEntry("SCPZombie", obj)
            end
            return
        end

        if obj:IsA("BasePart") then
            for cat, models in pairs(GANKZ_WorldReg) do
                for model, entry in pairs(models) do
                    if entry.part == obj then
                        GANKZ_RemoveWorldEntry(cat, model)
                    end
                end
            end
        end
    ...
    local function GANKZ_AttachESPRoot(root)
        if not root or GANKZ_MapAdd[root] then return end
        GANKZ_MapAdd[root] = root.DescendantAdded:Connect(GANKZ_RegisterWorldDescendant)
        GANKZ_MapRem[root] = root.DescendantRemoving:Connect(GANKZ_UnregisterWorldDescendant)
        for _, descendant in ipairs(root:GetDescendants()) do
            GANKZ_RegisterWorldDescendant(descendant)
        end
    end

    local function GANKZ_RefreshESPRoots()
        for _, conn in pairs(GANKZ_MapAdd) do
            if conn then pcall(function() conn:Disconnect() end) end
        end
        for _, conn in pairs(GANKZ_MapRem) do
            if conn then pcall(function() conn:Disconnect() end) end
        end
        GANKZ_MapAdd, GANKZ_MapRem = {}, {}
        ...
        if map then GANKZ_AttachESPRoot(map) end
        if map1 then GANKZ_AttachESPRoot(map1) end
    end

    local function GANKZ_LabelForPallet(model)
        local state = GANKZ_PalletState[model] or "UP"
    ...
    local function GANKZ_LabelForWindow(model)
        local state = GANKZ_WindowState[model] or "READY"
        if state == "BUSY" then return "Window (busy)" end
        return "Window"
    end

    local function GANKZ_AnyWorldEnabled()
        return GANKZ_ESPState.WorldMasterESP and (
            GANKZ_ESPState.GeneratorESP or
            GANKZ_ESPState.HookESP or
            GANKZ_ESPState.GateESP or
            GANKZ_ESPState.WindowESP or
            GANKZ_ESPState.PalletESP or
            GANKZ_ESPState.SCPZombieESP
        )
    end

    local function GANKZ_WorldCategoryData(cat)
        if cat == "Generator" then return GANKZ_ESPState.GeneratorESP, GANKZ_ESPState.GeneratorColor end
        if cat == "Hook" then return GANKZ_ESPState.HookESP, GANKZ_ESPState.HookColor end
        if cat == "Gate" then return GANKZ_ESPState.GateESP, GANKZ_ESPState.GateColor end
        if cat == "Window" then return GANKZ_ESPState.WindowESP, GANKZ_ESPState.WindowColor end
        if cat == "Palletwrong" then return GANKZ_ESPState.PalletESP, GANKZ_ESPState.PalletColor end
        if cat == "SCPZombie" then return GANKZ_ESPState.SCPZombieESP, GANKZ_ESPState.SCPZombieColor end
        return false, Color3.new(1, 1, 1)
    end

    local function GANKZ_UpdateWorldTag(cat, model, part, color)
        local key = GANKZ_WorldKey(cat, model)
        local tagName = key .. "_Tag"
        local folder = GANKZ_GetESPFolder()
        GANKZ_ClearPrefix(tagName, tagName)

        if not GANKZ_ValidPart(part) then
            GANKZ_DestroyChild(tagName)
            return
        end
        ...
        local root = LP.Character and LP.Character:FindFirstChild("HumanoidRootPart")
        local distanceText = ""
        if GANKZ_ESPState.WorldDistanceESP and root then
            distanceText = "[" .. tostring(math.floor((root.Position - part.Position).Magnitude)) .. "m]"
        end

        local nameText = ""
        local labelColor = color
        if GANKZ_ESPState.WorldNametags then
            if cat == "Generator" then
                local txt, genColor = GANKZ_GeneratorLabel(model)
                nameText = txt
                labelColor = genColor
            elseif cat == "Palletwrong" then
                nameText = GANKZ_LabelForPallet(model)
            elseif cat == "Window" then
                nameText = GANKZ_LabelForWindow(model)
            elseif cat == "SCPZombie" then
                nameText = model.Name
            else
                nameText = cat
    ...
    local function GANKZ_ClearAllWorldESP()
        for cat, models in pairs(GANKZ_WorldReg) do
            for model in pairs(models) do
                GANKZ_ClearWorldVisual(cat, model)
            end
    ...
    local function GANKZ_StartWorldLoop()
        if GANKZ_WorldLoopThread then return end
        GANKZ_WorldLoopThread = task.spawn(function()
            while not GANKZ_Dead and GANKZ_AnyWorldEnabled() do
                for cat, models in pairs(GANKZ_WorldReg) do
                    local enabled, color = GANKZ_WorldCategoryData(cat)
                    if enabled and GANKZ_ESPState.WorldMasterESP then
                        local n = 0
                        for model, entry in pairs(models) do
                            if cat == "Palletwrong" and GANKZ_IsPalletGone(model) then
                                GANKZ_RemoveWorldEntry(cat, model)
                            elseif model and GANKZ_Alive(model) then
                                local part = entry.part
                                if not GANKZ_ValidPart(part) or (model:IsA("Model") and not part:IsDescendantOf(model)) then
                                    entry.part = GANKZ_PickWorldPart(model, cat)
                                    part = entry.part
                                end

                                if GANKZ_ValidPart(part) then
                                    local key = GANKZ_WorldKey(cat, model)
                                    GANKZ_EnsureHighlight(key .. "_HL", model, color, false)
                                    GANKZ_UpdateWorldTag(cat, model, part, color)
                                else
                                    GANKZ_RemoveWorldEntry(cat, model)
                                end
                            else
            ...
            GANKZ_WorldLoopThread = nil
        end)
    end

    local function GANKZ_Selected(selected, name)
    ...
    getgenv().GANKZ_AddVisualESPControls = function(VisualTabRef)
        if not VisualTabRef or GANKZ_ControlsAdded then return end
        GANKZ_ControlsAdded = true
        ...
        settingsSection:AddSlider({
            ...
            Flag = "GANKZ ESP Fill Transparency",
            Min = 0,
            Max = 1,
            Default = GANKZ_ESPState.ESPFillTransparency,
            Increment = 0.01,
            Callback = function(value)
                GANKZ_ESPState.ESPFillTransparency = value
                GANKZ_RefreshAllPlayers()
            end,
        })
        ...
        settingsSection:AddSlider({
            ...
            Flag = "GANKZ ESP Outline Transparency",
            Min = 0,
            Max = 1,
            Default = GANKZ_ESPState.ESPOutlineTransparency,
            Increment = 0.01,
            Callback = function(value)
                GANKZ_ESPState.ESPOutlineTransparency = value
                GANKZ_RefreshAllPlayers()
            end,
        })
        ...
        settingsSection:AddSlider({
            Name = "ESP Text Size",
            Flag = "GANKZ ESP Text Size",
            Min = 8,
            Max = 22,
            Default = GANKZ_ESPState.ESPTextSize,
            Increment = 1,
            Callback = function(value)
                GANKZ_ESPState.ESPTextSize = value
                GANKZ_RefreshAllPlayers()
            end,
        })
        ...
        playerSection:AddToggle({
            ...
            Flag = "GANKZ Enable Player ESP",
            Default = false,
            Callback = function(state)
                GANKZ_ESPState.PlayerMasterESP = state
                if state then
                    GANKZ_StartPlayerLoop()
                    GANKZ_RefreshAllPlayers()
                else
                    GANKZ_ClearAllPlayerESP()
                end
            end,
        })
        ...
        playerSection:AddDropdown({
            ...
            Flag = "GANKZ Select Player ESP",
            Values = { "Survivor ESP", "Killer ESP", "Spectator ESP", "Survivor Items ESP" },
            Multi = true,
            AllowNone = true,
            Default = {},
            Callback = function(selected)
                GANKZ_ESPState.SurvivorESP = GANKZ_Selected(selected, "Survivor ESP")
                GANKZ_ESPState.KillerESP = GANKZ_Selected(selected, "Killer ESP")
                GANKZ_ESPState.SpectatorESP = GANKZ_Selected(selected, "Spectator ESP")
                GANKZ_ESPState.SurvivorItemsESP = GANKZ_Selected(selected, "Survivor Items ESP")

                if GANKZ_ESPState.PlayerMasterESP then
                    GANKZ_StartPlayerLoop()
                    GANKZ_RefreshAllPlayers()
                else
                    GANKZ_ClearAllPlayerESP()
        ...
        playerSection:AddToggle({
            ...
            Flag = "GANKZ Player Nametags",
            Default = false,
            Callback = function(state)
                GANKZ_ESPState.Nametags = state
                if GANKZ_ESPState.PlayerMasterESP then
                    GANKZ_StartPlayerLoop()
                    GANKZ_RefreshAllPlayers()
                else
        ...
        playerSection:AddToggle({
            ...
            Flag = "GANKZ Player Distance ESP",
            Default = false,
            Callback = function(state)
                GANKZ_ESPState.DistanceESP = state
                if GANKZ_ESPState.PlayerMasterESP then
                    GANKZ_StartPlayerLoop()
                    GANKZ_RefreshAllPlayers()
                else
        ...
        playerSection:AddColorPicker({ Name = "Survivor Color", Flag = "GANKZ Survivor Color", Default = GANKZ_ESPState.SurvivorColor, Callback = function(color) GANKZ_ESPState.SurvivorColor = color; GANKZ_RefreshAllPlayers() end })
        playerSection:AddColorPicker({ Name = "Killer Color", Flag = "GANKZ Killer Color", Default = GANKZ_ESPState.KillerColor, Callback = function(color) GANKZ_ESPState.KillerColor = color; GANKZ_RefreshAllPlayers() end })
        playerSection:AddColorPicker({ Name = "Spectator Color", Flag = "GANKZ Spectator Color", Default = GANKZ_ESPState.SpectatorColor, Callback = function(color) GANKZ_ESPState.SpectatorColor = color; GANKZ_RefreshAllPlayers() end })
        ...
        worldSection:AddToggle({
            ...
            Flag = "GANKZ Enable World ESP",
            Default = false,
            Callback = function(state)
                GANKZ_ESPState.WorldMasterESP = state
                if state then
                    GANKZ_RefreshESPRoots()
                    if GANKZ_AnyWorldEnabled() then GANKZ_StartWorldLoop() end
                else
                    GANKZ_ClearAllWorldESP()
                end
            end,
        })
        ...
        worldSection:AddDropdown({
            ...
            Flag = "GANKZ Select World Objects",
            Values = { "Generators", "Hooks", "Gates", "Windows", "Pallets", "SCP / Zombie" },
            Multi = true,
            AllowNone = true,
            Default = {},
            Callback = function(selected)
                GANKZ_ESPState.GeneratorESP = GANKZ_Selected(selected, "Generators")
                GANKZ_ESPState.HookESP = GANKZ_Selected(selected, "Hooks")
                GANKZ_ESPState.GateESP = GANKZ_Selected(selected, "Gates")
                GANKZ_ESPState.WindowESP = GANKZ_Selected(selected, "Windows")
                GANKZ_ESPState.PalletESP = GANKZ_Selected(selected, "Pallets")
                GANKZ_ESPState.SCPZombieESP = GANKZ_Selected(selected, "SCP / Zombie")

                if GANKZ_ESPState.WorldMasterESP and GANKZ_AnyWorldEnabled() then
                    GANKZ_RefreshESPRoots()
                    GANKZ_StartWorldLoop()
                else
                    GANKZ_ClearAllWorldESP()
                end
            end,
        ...
        worldSection:AddToggle({
            Name = "World Nametags",
            Flag = "GANKZ World Nametags",
            Default = false,
            Callback = function(state)
                GANKZ_ESPState.WorldNametags = state
                if GANKZ_ESPState.WorldMasterESP and GANKZ_AnyWorldEnabled() then GANKZ_StartWorldLoop() else GANKZ_ClearAllWorldESP() end
            end,
        })

        worldSection:AddToggle({
            Name = "World Distance ESP",
            Flag = "GANKZ World Distance ESP",
            Default = false,
            Callback = function(state)
                GANKZ_ESPState.WorldDistanceESP = state
                if GANKZ_ESPState.WorldMasterESP and GANKZ_AnyWorldEnabled() then GANKZ_StartWorldLoop() else GANKZ_ClearAllWorldESP() end
            end,
        })

        pcall(function() worldSection:AddDivider({ Text = "Colors" }) end)
        worldSection:AddColorPicker({ Name = "Generator Color", Flag = "GANKZ Generator Color", Default = GANKZ_ESPState.GeneratorColor, Callback = function(color) GANKZ_ESPState.GeneratorColor = color end })
        worldSection:AddColorPicker({ Name = "Hook Color", Flag = "GANKZ Hook Color", Default = GANKZ_ESPState.HookColor, Callback = function(color) GANKZ_ESPState.HookColor = color end })
        worldSection:AddColorPicker({ Name = "Gate Color", Flag = "GANKZ Gate Color", Default = GANKZ_ESPState.GateColor, Callback = function(color) GANKZ_ESPState.GateColor = color end })
        worldSection:AddColorPicker({ Name = "Window Color", Flag = "GANKZ Window Color", Default = GANKZ_ESPState.WindowColor, Callback = function(color) GANKZ_ESPState.WindowColor = color end })
        worldSection:AddColorPicker({ Name = "Pallet Color", Flag = "GANKZ Pallet Color", Default = GANKZ_ESPState.PalletColor, Callback = function(color) GANKZ_ESPState.PalletColor = color end })
        worldSection:AddColorPicker({ Name = "SCP / Zombie Color", Flag = "GANKZ SCP Zombie Color", Default = GANKZ_ESPState.SCPZombieColor, Callback = function(color) GANKZ_ESPState.SCPZombieColor = color end })
    end

    for _, player in ipairs(Players:GetPlayers()) do
        GANKZ_WatchPlayer(player)
    end

    table.insert(GANKZ_Connections, Players.PlayerAdded:Connect(GANKZ_WatchPlayer))
    table.insert(GANKZ_Connections, Players.PlayerRemoving:Connect(GANKZ_UnwatchPlayer))
    table.insert(GANKZ_Connections, Workspace.ChildAdded:Connect(function(child)
        if child.Name == "Map" or child.Name == "Map1" then
            GANKZ_AttachESPRoot(child)
            if GANKZ_ESPState.WorldMasterESP and GANKZ_AnyWorldEnabled() then GANKZ_StartWorldLoop() end
        end
    end))
    table.insert(GANKZ_Connections, Workspace.ChildRemoved:Connect(function(child)
        if child.Name == "Map" or child.Name == "Map1" then
            GANKZ_RefreshESPRoots()
        end
    end))
    ...
    getgenv().GANKZ_VD_VisualESP_Cleanup = function()
        GANKZ_Dead = true
        GANKZ_ClearAllPlayerESP()
        GANKZ_ClearAllWorldESP()

        for _, conn in ipairs(GANKZ_Connections) do
            if conn then pcall(function() conn:Disconnect() end) end
        end
        for _, conns in pairs(GANKZ_PlayerConns) do
        ...
        if GANKZ_ESPFolder and GANKZ_ESPFolder.Parent then
            GANKZ_ESPFolder:Destroy()
        end
    end

    GANKZ_SafeNotify("Visual ESP", "Highlight ESP V2 loaded. Anti double nametag aktif.", 3)
end
...
task.spawn(function()
    while not VD.Destroyed do
        if VD.Fullbright then
            local weatherTheme = VD.VIS_WeatherTheme and GANKZ_WeatherPresets[VD.VIS_WeatherTheme]
            ...
            if VD.VIS_WeatherTheme and VD.VIS_WeatherTheme ~= "Default" and GANKZ_WeatherPresets[VD.VIS_WeatherTheme] then
                local theme = GANKZ_WeatherPresets[VD.VIS_WeatherTheme]
                if theme.Lighting then
                    for k, v in pairs(theme.Lighting) do
                        pcall(function() Lighting[k] = v end)
                    end
...
local function GANKZ_ToFIsTargetVisible(originPos, targetPos, targetCharacter)
    ...
    if GANKZ_ToFState.LaserBeam then table.insert(excludeList, GANKZ_ToFState.LaserBeam) end
...
local function GANKZ_ToFGetTargetPosition()
    ...
    local function predictTarget(torso, targetCharacter)
        ...
        if VD.TOF_WallCheck and not GANKZ_ToFIsTargetVisible(originPos, targetPos, targetCharacter) then
...
local function GANKZ_ToFUpdateLaser(originPos, targetPos)
    ...
    GANKZ_ToFState.LaserBeam.Size = Vector3.new(0.05, 0.05, dist)
    GANKZ_ToFState.LaserBeam.CFrame = CFrame.new((originPos + targetPos) / 2, targetPos)
    GANKZ_ToFState.LaserBeam.Transparency = 0
end

local function GANKZ_ToFClearLaser()
    if GANKZ_ToFState.LaserBeam then
        pcall(function() GANKZ_ToFState.LaserBeam:Destroy() end)
...
local function GANKZ_ToFDoShoot()
    ...
    local targetDirection, gunObject, originPos, targetPos = GANKZ_ToFGetTargetPosition()
...
local function GANKZ_ToFCreateTargetSelectorUI()
    ...
    if not parent then return end
    if GANKZ_ToFState.TargetGui and GANKZ_ToFState.TargetGui.Parent then return end
    ...
    for i, mode in ipairs(modes) do
        ...
        btn.MouseButton1Click:Connect(function()
            GANKZ_ToFSetTargetMode(mode.Internal, false)
        end)
...
local function GANKZ_ToFDestroyTargetSelectorUI()
    if GANKZ_ToFState.TargetGui then
        pcall(function() GANKZ_ToFState.TargetGui:Destroy() end)
...
local function GANKZ_ToFStartConnection()
    if GANKZ_ToFState.Connection then return end
    GANKZ_ToFState.Connection = RunService.Heartbeat:Connect(function()
        if not VD.TOF_SilentAim or not GANKZ_ToFState.IsAiming then
            if GANKZ_ToFState.LaserBeam then GANKZ_ToFState.LaserBeam.Transparency = 1 end
            return
        end

        local _, _, originPos, targetPos = GANKZ_ToFGetTargetPosition()
        if originPos and targetPos then
            ...
            if VD.TOF_Laser then
                GANKZ_ToFUpdateLaser(originPos, targetPos)
...
local function GANKZ_ToFStopConnection()
    if GANKZ_ToFState.Connection then
        pcall(function() GANKZ_ToFState.Connection:Disconnect() end)
...
local function GANKZ_ToFDisconnectInputs()
    if GANKZ_ToFState.InputBegan then pcall(function() GANKZ_ToFState.InputBegan:Disconnect() end) end
    if GANKZ_ToFState.InputEnded then pcall(function() GANKZ_ToFState.InputEnded:Disconnect() end) end
    GANKZ_ToFState.InputBegan = nil
    GANKZ_ToFState.InputEnded = nil
end
...
local function GANKZ_ToFEnsureInputs()
    if not GANKZ_ToFState.InputBegan then
        GANKZ_ToFState.InputBegan = UserInputService.InputBegan:Connect(function(input, gameProcessed)
            if gameProcessed then return end

            local keyCode = GANKZ_ToFKeyCodes[VD.TOF_Key or "None"]
            if keyCode and input.UserInputType == Enum.UserInputType.Keyboard and input.KeyCode == keyCode then
                GANKZ_SetToFSilentAim(not VD.TOF_SilentAim)
                return
            end

            if not VD.TOF_SilentAim then return end
            if input.UserInputType == Enum.UserInputType.MouseButton1
            or (input.UserInputType == Enum.UserInputType.Touch and GANKZ_ToFIsTouchOnShootButton(input)) then
            ...
            if input.UserInputType == Enum.UserInputType.Keyboard then
                if input.KeyCode == Enum.KeyCode.K then
                    GANKZ_ToFSetTargetMode("Killer", true)
                elseif input.KeyCode == Enum.KeyCode.J then
                    GANKZ_ToFSetTargetMode("Survivors", true)
                elseif input.KeyCode == Enum.KeyCode.L then
                    GANKZ_ToFSetTargetMode("Zombie", true)
                end
            end
        end)
    end
    if not GANKZ_ToFState.InputEnded then
        GANKZ_ToFState.InputEnded = UserInputService.InputEnded:Connect(function(input)
            if input.UserInputType == Enum.UserInputType.MouseButton1
            or (input.UserInputType == Enum.UserInputType.Touch and input == GANKZ_ToFState.TouchInput) then
                GANKZ_ToFState.IsAiming = false
                if input == GANKZ_ToFState.TouchInput then GANKZ_ToFState.TouchInput = nil end
                if GANKZ_ToFState.LaserBeam then GANKZ_ToFState.LaserBeam.Transparency = 1 end
            end
        end)
    end
...
GANKZ_ToFEnsureInputs()
getgenv().GANKZ_SetToFSilentAim = GANKZ_SetToFSilentAim
getgenv().GANKZ_ToFClearLaser = GANKZ_ToFClearLaser
getgenv().GANKZ_ToFSetTargetMode = GANKZ_ToFSetTargetMode
end)();
...
-- =====================================================
(function()
local GANKZ_HideSurvivorIconState = {
    Connection = nil,
    Originals = {},
}

local GANKZ_HideSurvivorIconImage = "rbxassetid://95802749943553"
local GANKZ_HideSurvivorIconText = "NxH"

local function GANKZ_GetSurvivorSlots()
    local slots = {}
    local playerGui = LocalPlayer:FindFirstChild("PlayerGui")
...
local function GANKZ_ApplyHideSurvivorIcon()
    for _, slot in ipairs(GANKZ_GetSurvivorSlots()) do
        local imageLabel = slot.ImageLabel
        if imageLabel and imageLabel:IsA("ImageLabel") then
            if not GANKZ_HideSurvivorIconState.Originals[imageLabel] then
                GANKZ_HideSurvivorIconState.Originals[imageLabel] = {
            ...
            imageLabel.Image = GANKZ_HideSurvivorIconImage
        ...
        if textLabel and textLabel:IsA("TextLabel") then
            if not GANKZ_HideSurvivorIconState.Originals[textLabel] then
                GANKZ_HideSurvivorIconState.Originals[textLabel] = {
                    Text = textLabel.Text,
            ...
            textLabel.Text = GANKZ_HideSurvivorIconText
...
local function GANKZ_RestoreSurvivorIcons()
    for object, original in pairs(GANKZ_HideSurvivorIconState.Originals) do
    ...
    GANKZ_HideSurvivorIconState.Originals = {}
end

local function GANKZ_SetHideSurvivorIcon(enabled)
    VD.VIS_HideSurvivorIcon = enabled and true or false

    if VD.VIS_HideSurvivorIcon then
        GANKZ_ApplyHideSurvivorIcon()
        if not GANKZ_HideSurvivorIconState.Connection then
            GANKZ_HideSurvivorIconState.Connection = RunService.Heartbeat:Connect(function()
                if VD.VIS_HideSurvivorIcon then
                    GANKZ_ApplyHideSurvivorIcon()
                end
            end)
        ...
        if GANKZ_HideSurvivorIconState.Connection then
            pcall(function() GANKZ_HideSurvivorIconState.Connection:Disconnect() end)
            GANKZ_HideSurvivorIconState.Connection = nil
        end
        GANKZ_RestoreSurvivorIcons()
    end
end

getgenv().GANKZ_SetHideSurvivorIcon = GANKZ_SetHideSurvivorIcon
end)();
...
-- =====================================================
(function()
local GANKZ_HookCounterState = {
    Connection = nil,
}

local function GANKZ_UpdateHookCounter(enabled)
    local playerGui = LocalPlayer:FindFirstChild("PlayerGui")
            ...
            if frame then
                for i = 1, 5 do
                    ...
                    if imageLabel and textLabel then
                        ...
                        local labelName = "GANKZ_CustomHookCounter"
...
local function GANKZ_SetShowHookCounter(enabled)
    VD.VIS_ShowHookCounter = enabled and true or false

    if VD.VIS_ShowHookCounter then
        GANKZ_UpdateHookCounter(true)
        if not GANKZ_HookCounterState.Connection then
            GANKZ_HookCounterState.Connection = RunService.Heartbeat:Connect(function()
                if VD.VIS_ShowHookCounter then
                    GANKZ_UpdateHookCounter(true)
                end
            end)
        ...
        if GANKZ_HookCounterState.Connection then
            pcall(function() GANKZ_HookCounterState.Connection:Disconnect() end)
            GANKZ_HookCounterState.Connection = nil
        end
        GANKZ_UpdateHookCounter(false)
                ...
                if gui:IsA("ScreenGui") and gui.Name:match("%-mob$") then
                    ...
                    if frame then
                        for i = 1, 5 do
                            local survivorFrame = frame:FindFirstChild("Survivor" .. i)
                            local imageLabel = survivorFrame and survivorFrame:FindFirstChild("ImageLabel")
                            customLabel = imageLabel and imageLabel:FindFirstChild("GANKZ_CustomHookCounter")
                            if customLabel then
...
getgenv().GANKZ_SetShowHookCounter = GANKZ_SetShowHookCounter
end)();
...
local function GANKZ_GetPingValue()
...
local function GANKZ_CreatePingFPSGui()
    local parent = GetSafeGuiParent()
    if not parent then return nil end

    local old = parent:FindFirstChild("GANKZ_PingFPSGui")
    if old then pcall(function() old:Destroy() end) end

    local sg = Instance.new("ScreenGui")
    sg.Name = "GANKZ_PingFPSGui"
...
local function GANKZ_SetShowPingFPS(enabled)
    VD.VIS_ShowPingFPS = enabled and true or false

    if VD.VIS_ShowPingFPS then
        GANKZ_PingFPSState.Gui = GANKZ_PingFPSState.Gui or GANKZ_CreatePingFPSGui()
        GANKZ_PingFPSState.Frames = 0
        GANKZ_PingFPSState.LastUpdate = tick()

        if not GANKZ_PingFPSState.Connection then
            GANKZ_PingFPSState.Connection = RunService.RenderStepped:Connect(function()
                if not VD.VIS_ShowPingFPS then return end

                GANKZ_PingFPSState.Frames = GANKZ_PingFPSState.Frames + 1
                local now = tick()
                if now - GANKZ_PingFPSState.LastUpdate < 0.5 then return end

                local fps = math.floor(GANKZ_PingFPSState.Frames / (now - GANKZ_PingFPSState.LastUpdate) + 0.5)
                local ping = GANKZ_GetPingValue()
                GANKZ_PingFPSState.Frames = 0
                GANKZ_PingFPSState.LastUpdate = now

                if not (GANKZ_PingFPSState.Gui and GANKZ_PingFPSState.Gui.Parent) then
                    GANKZ_PingFPSState.Gui = GANKZ_CreatePingFPSGui()
                end

                local label = GANKZ_PingFPSState.Gui and GANKZ_PingFPSState.Gui:FindFirstChild("PingFPSLabel", true)
                if label then
        ...
        if GANKZ_PingFPSState.Connection then
            pcall(function() GANKZ_PingFPSState.Connection:Disconnect() end)
            GANKZ_PingFPSState.Connection = nil
        end
        if GANKZ_PingFPSState.Gui then
            pcall(function() GANKZ_PingFPSState.Gui:Destroy() end)
            GANKZ_PingFPSState.Gui = nil
        end
    end
end

getgenv().GANKZ_SetShowPingFPS = GANKZ_SetShowPingFPS
end)();
...
-- =====================================================
(function()
local GANKZ_FlashlightAimState = {
    Connection = nil,
    LaserBeam = nil,
    FlashlightPart = nil,
    Active = false,
...
local function GANKZ_GetFlashlightActivateRemote()
    local remotes = ReplicatedStorage:FindFirstChild("Remotes")
    local items = remotes and remotes:FindFirstChild("Items")
...
local function GANKZ_GetFlashlightTargetPart(char)
    if not char then return nil end
...
local function GANKZ_IsAliveCharacter(char)
...
local function GANKZ_GetFlashlightTarget()
    local localChar = LocalPlayer.Character
    ...
    for _, player in ipairs(Players:GetPlayers()) do
        if player ~= LocalPlayer and player.Character and GANKZ_IsAliveCharacter(player.Character) then
            local isKiller = player.Team and player.Team.Name == "Killer"
            if isKiller then
                local part = GANKZ_GetFlashlightTargetPart(player.Character)
                if part then
...
local function GANKZ_ClearFlashlightLaser()
    if GANKZ_FlashlightAimState.LaserBeam then
        pcall(function() GANKZ_FlashlightAimState.LaserBeam:Destroy() end)
        GANKZ_FlashlightAimState.LaserBeam = nil
    end
end

local function GANKZ_GetFlashlightOrigin(cam)
    local source = GANKZ_FlashlightAimState.FlashlightPart
...
local function GANKZ_UpdateFlashlightLaser(originPos, targetPos)
    if not GANKZ_FlashlightAimState.LaserBeam then
        local laser = Instance.new("Part")
        ...
        laser.Parent = workspace
        GANKZ_FlashlightAimState.LaserBeam = laser
    end

    local dist = (targetPos - originPos).Magnitude
    if dist < 0.1 then return end

    local laser = GANKZ_FlashlightAimState.LaserBeam
    laser.Size = Vector3.new(0.16, 0.16, dist)
    laser.CFrame = CFrame.new((originPos + targetPos) / 2, targetPos)
    laser.Transparency = 0
end

local function GANKZ_FlashlightAimStep()
    if false then
        if GANKZ_FlashlightAimState.LaserBeam then
            GANKZ_FlashlightAimState.LaserBeam.Transparency = 1
        end
        return
    end

    if not (VD.FLASH_SilentAim and GANKZ_FlashlightAimState.Active) then
        if GANKZ_FlashlightAimState.LaserBeam then
            GANKZ_FlashlightAimState.LaserBeam.Transparency = 1
        end
        return
    ...
    local cam = workspace.CurrentCamera
    local targetPart = GANKZ_GetFlashlightTarget()
    ...
    local originPos = GANKZ_GetFlashlightOrigin(cam)

    if VD.FLASH_Laser and originPos then
        GANKZ_UpdateFlashlightLaser(originPos, targetPos)
    elseif GANKZ_FlashlightAimState.LaserBeam then
        GANKZ_FlashlightAimState.LaserBeam.Transparency = 1
    end
...
local function GANKZ_StartFlashlightSilentAim()
    getgenv().GANKZ_FlashlightActivateRemote = GANKZ_GetFlashlightActivateRemote()
    if GANKZ_FlashlightAimState.Connection then return end
    GANKZ_FlashlightAimState.Connection = RunService.RenderStepped:Connect(GANKZ_FlashlightAimStep)
end

local function GANKZ_StopFlashlightSilentAim()
    GANKZ_FlashlightAimState.Active = false
    GANKZ_FlashlightAimState.FlashlightPart = nil
    GANKZ_ClearFlashlightLaser()
    if GANKZ_FlashlightAimState.Connection then
        pcall(function() GANKZ_FlashlightAimState.Connection:Disconnect() end)
        GANKZ_FlashlightAimState.Connection = nil
    end
end

local function GANKZ_SetFlashlightSilentAim(enabled)
    if enabled and false then
        VD.FLASH_SilentAim = false
        GANKZ_StopFlashlightSilentAim()
        return
    end

    VD.FLASH_SilentAim = enabled and true or false
    if VD.FLASH_SilentAim then
        GANKZ_StartFlashlightSilentAim()
    else
        GANKZ_StopFlashlightSilentAim()
    end
end

getgenv().GANKZ_SetFlashlightSilentAim = GANKZ_SetFlashlightSilentAim
getgenv().GANKZ_ClearFlashlightLaser = GANKZ_ClearFlashlightLaser
getgenv().GANKZ_SetFlashlightAimActive = function(active, flashlightPart)
    GANKZ_FlashlightAimState.Active = active and true or false
    if GANKZ_FlashlightAimState.Active and flashlightPart then
        GANKZ_FlashlightAimState.FlashlightPart = flashlightPart
    elseif not GANKZ_FlashlightAimState.Active then
        GANKZ_FlashlightAimState.FlashlightPart = nil
    end
    if not GANKZ_FlashlightAimState.Active and GANKZ_FlashlightAimState.LaserBeam then
        GANKZ_FlashlightAimState.LaserBeam.Transparency = 1
    end
end
getgenv().GANKZ_FlashlightActivateRemote = GANKZ_GetFlashlightActivateRemote()
...
VD_ParryRange.Name = "GANKZ_ParryRange"
    ...
    if VeilState.chargingSpear and VeilConfig.Enabled and isSpearMode then
        ...
        if target and target.Part and target.Part.Parent then
            ...
            if VeilConfig.ShowTargetLaser then
                if not getgenv().GANKZ_SpearLaserPart then
                    local laser = Instance.new("Part")
                    ...
                    laser.Transparency = 0
                    laser.Parent = workspace
                    getgenv().GANKZ_SpearLaserPart = laser
                end
                ...
                if originPart then
                    ...
                    local dist = (targetPos - originPos).Magnitude
                    if dist > 0.1 then
                        local laser = getgenv().GANKZ_SpearLaserPart
                        laser.Size = Vector3.new(0.16, 0.16, dist)
                        laser.CFrame = CFrame.new((originPos + targetPos) / 2, targetPos)
                ...
                if getgenv().GANKZ_SpearLaserPart then getgenv().GANKZ_SpearLaserPart.Transparency = 1 end
            end
        else
...
local GANKZ_MainInfoPanel = {
    Widgets = {},
    Texts = {},
}

function GANKZ_InfoPlainText(text)
...
function GANKZ_UpdateInfoWidget(widget, text)
...
function GANKZ_SetMainInfoPanelText(key, title, text)
    local value = tostring(title or key) .. "\n" .. GANKZ_InfoPlainText(text)
    GANKZ_MainInfoPanel.Texts[key] = value
    GANKZ_UpdateInfoWidget(GANKZ_MainInfoPanel.Widgets[key], value)
end

function GANKZ_RegisterMainInfoWidget(key, widget)
    GANKZ_MainInfoPanel.Widgets[key] = widget
    if GANKZ_MainInfoPanel.Texts[key] then
        GANKZ_UpdateInfoWidget(widget, GANKZ_MainInfoPanel.Texts[key])
    end
end

function GANKZ_AddMainInfoLine(section, key, title, defaultText)
    local defaultValue = tostring(title) .. "\n" .. tostring(defaultText or "Off")
    GANKZ_MainInfoPanel.Texts[key] = GANKZ_MainInfoPanel.Texts[key] or defaultValue
    ...
    if ok and widget then return GANKZ_RegisterMainInfoWidget(key, widget) end
...
do -- ESP Tab
    ...
    pcall(function()
        if getgenv().GANKZ_AddVisualESPControls then
            getgenv().GANKZ_AddVisualESPControls(VisualTab)
        end
    end)
...
do -- Aim Tab
    ...
    flaskSection:AddToggle({ Default = false, Name = "Flask Laser (Cure)", Locked = false, TextLocked = "Premium Required", Flag = "Flask Laser (Cure)", Callback = function(v)
        ...
        if v then
            pcall(GANKZ_StartCureFlaskLaser)
        else
            if getgenv().GANKZ_CureFlaskLaserThread then
                getgenv().GANKZ_CureFlaskLaserThread:Disconnect()
                getgenv().GANKZ_CureFlaskLaserThread = nil
            end
            if getgenv().GANKZ_CureFlaskLaserPart then
                pcall(function() getgenv().GANKZ_CureFlaskLaserPart:Destroy() end)
                getgenv().GANKZ_CureFlaskLaserPart = nil
            end
        end
    end })
    ...
    tofSection:AddToggle({
        ...
        Callback = function(v)
            ...
            VD.TOF_Laser = v
            if not v and getgenv().GANKZ_ToFClearLaser then
    ...
    tofSection:AddDropdown({
        ...
        Callback = function(v)
            if type(v) == "table" then v = v[1] end
            if getgenv().GANKZ_ToFSetTargetMode then
                getgenv().GANKZ_ToFSetTargetMode(v or "Killer", false)
            else
    ...
    flashlightSection:AddToggle({
        ...
        Callback = function(v)
            ...
            if getgenv().GANKZ_SetFlashlightSilentAim then
                getgenv().GANKZ_SetFlashlightSilentAim(v)
            else
    ...
    flashlightSection:AddToggle({
        ...
        Callback = function(v)
            ...
            VD.FLASH_Laser = v
            if not v and getgenv().GANKZ_ClearFlashlightLaser then
                getgenv().GANKZ_ClearFlashlightLaser()
            end
        end
    })
...
do -- FOV Tab
    ...
    GANKZ_AddMainInfoLine(infoPanelSection, "GanKunZKiller", "GanKunZ Killer Display", "Off")
    GANKZ_AddMainInfoLine(infoPanelSection, "KillerPerks", "Spectate Killer Perks", "Off")
    GANKZ_AddMainInfoLine(infoPanelSection, "PredictMap", "Predict Map", "Off")

    visualSection:AddToggle({ Default = false, Name = "GanKunZ Killer Display", Flag = "GanKunZ Killer Display", Callback = function(v) 
        VD.VIS_GanKunZKiller = v 
        if v then
            StartGanKunZKiller()
        else
            StopGanKunZKiller()
    ...
    visualSection:AddToggle({ Default = false, Name = "Hide Survivor Icon", Flag = "Hide Survivor Icon", Callback = function(v)
        if getgenv().GANKZ_SetHideSurvivorIcon then
            getgenv().GANKZ_SetHideSurvivorIcon(v)
        else
            VD.VIS_HideSurvivorIcon = v
        end
    end })
    visualSection:AddToggle({ Default = false, Name = "Show Ping & FPS", Flag = "Show Ping & FPS", Callback = function(v)
        if getgenv().GANKZ_SetShowPingFPS then
            getgenv().GANKZ_SetShowPingFPS(v)
        else
    ...
    visualSection:AddToggle({ Default = false, Name = "Show Hook Counter", Flag = "Show Hook Counter", Callback = function(v)
        if getgenv().GANKZ_SetShowHookCounter then
            getgenv().GANKZ_SetShowHookCounter(v)
        else
...
getgenv().GANKZ_FakeParryTrack = nil
...
function VD_PlayFakeParry()
    ...
    pcall(function()
        ...
        if getgenv().GANKZ_FakeParryTrack then
            pcall(function() getgenv().GANKZ_FakeParryTrack:Stop() end)
            getgenv().GANKZ_FakeParryTrack = nil
        end
        ...
        track:Play()
        getgenv().GANKZ_FakeParryTrack = track
    end)
end

if not getgenv().GANKZ_FakeParryInputConn then
    getgenv().GANKZ_FakeParryInputConn = game:GetService("UserInputService").InputBegan:Connect(function(input, gp)
        if gp then return end
...
-- ==========================================
getgenv().GANKZ_FakeGenTrack = nil
FakeGenData = {
    UI = nil,
    Button = nil,
    DragLocked = false,
...
function VD_ToggleFakeGen()
    if not VD.SURV_FakeGen then
        if getgenv().GANKZ_FakeGenTrack then
            pcall(function() getgenv().GANKZ_FakeGenTrack:Stop() end)
            getgenv().GANKZ_FakeGenTrack = nil
        end
    ...
    if getgenv().GANKZ_FakeGenTrack then
        ...
        pcall(function()
            ...
            track.Priority = Enum.AnimationPriority.Action
            track:Play()
            getgenv().GANKZ_FakeGenTrack = track
...
do -- Killer Tab
    ...
    abilityKiller:AddToggle({ Default = false, Name = "Infinite Abyssal Burst (Abyss)", Locked = false, TextLocked = "Premium Required", Flag = "Infinite Abyssal Burst (Abyss)", Callback = function(v)
        ...
        if v then
            GANKZ_StartAbyssCooldownBypass()
        else
            GANKZ_StopAbyssCooldownBypass()
        end
    end })
    abilityKiller:AddToggle({ Default = false, Name = "Infinite Skill (Hidden)", Locked = false, TextLocked = "Premium Required", Flag = "Infinite Skill (Hidden)", Callback = function(v)
        ...
        if v then
            pcall(GANKZ_StartHiddenCooldownBypass)
        else
            pcall(GANKZ_StopHiddenCooldownBypass)
        end
    end })
    abilityKiller:AddToggle({ Default = false, Name = "Infinite Frenzy (Jeff)", Locked = false, TextLocked = "Premium Required", Flag = "Infinite Frenzy (Jeff)", Callback = function(v)
        ...
        if v then
            pcall(GANKZ_StartJeffCooldownBypass)
        else
            pcall(GANKZ_StopJeffCooldownBypass)
        end
    end })
    ...
    abilityKiller:AddToggle({ Default = false, Name = "Infinite Lake Mist (Jason)", Locked = false, TextLocked = "Premium Required", Flag = "Infinite Lake Mist (Jason)", Callback = function(v)
        ...
        if v then
            pcall(GANKZ_StartSlasherCooldownBypass)
        else
            pcall(GANKZ_StopSlasherCooldownBypass)
        end
    end })
    ...
    abilityKiller:AddToggle({ Default = false, Name = "Fake Attack (Counter Parry)", Locked = false, TextLocked = "Premium Required", Flag = "Fake Attack (Counter Parry)", Callback = function(v)
        ...
        pcall(GANKZ_ToggleFakeAttack, v)
    end })
    ...
    pcall(function()
        ...
        abilityKiller:AddButton({
            ...
            Callback = function()
                ...
                pcall(GANKZ_ApplyCustomMasked, VD.KILLER_CustomMasked)
            end
        })
        abilityKiller:AddButton({
            ...
            Callback = function()
                ...
                pcall(GANKZ_ApplyCustomMasked, mask)
            end
        })
    end)
...
do -- Fling Tab
    ...
    flingSection:AddButton({ Name = "Fling Nearest", Locked = false, TextLocked = "Premium Required", Callback = function() 
        ...
        pcall(function() GANKZ_FlingNearest() end) 
    end })
...
print("GanKunZ Hub Violence District v1.5.7 Loaded")
pcall(function()
    VD_Notify("GanKunZ Hub", "Violence District v1.5.7 Loaded Successfully!", 5)
end)
...
function GANKZ_ApplyCustomMasked(maskName)
...
function VD_ClearSurvivorWarnings()
    for _, player in ipairs(Players:GetPlayers()) do
        ...
        local root = char and char:FindFirstChild("HumanoidRootPart")
        local warn = root and root:FindFirstChild("GANKZ_SurvivorKillerWarn")
...
function VD_UpdateSurvivorWarnings()
    ...
    local now = tick()
    if VD._WarnKillerGanKunZ and now < VD._WarnKillerGanKunZ then return end
    VD._WarnKillerGanKunZ = now + 0.15
    ...
    for _, player in ipairs(Players:GetPlayers()) do
        if IsSurvivor(player) and player.Character then
            ...
            if root and hum and hum.Health > 0 then
                ...
                local warn = root:FindFirstChild("GANKZ_SurvivorKillerWarn")
                if nearest <= 60 then
                    local danger = nearest <= 40
                    if not warn then
                        warn = Instance.new("BillboardGui")
                        warn.Name = "GANKZ_SurvivorKillerWarn"
                        warn.Adornee = root
...
function VD_UpdateBypassGate()
    ...
    if VD._GanKunZBypassGate and tick() < VD._GanKunZBypassGate then return end
    VD._GanKunZBypassGate = tick() + 1
...
function VD_CreateMoonwalkButton()
    ...
    local old = parent:FindFirstChild("GANKZ_MoonwalkButton")
    if old then pcall(function() old:Destroy() end) end

    local sg = Instance.new("ScreenGui")
    sg.Name = "GANKZ_MoonwalkButton"
    sg.ResetOnSpawn = false
    sg.IgnoreGuiInset = true
...
local function VD_CreateAimLockButton()
    ...
    local old = parent:FindFirstChild("GANKZ_AimLockButton")
    if old then pcall(function() old:Destroy() end) end

    local sg = Instance.new("ScreenGui")
    sg.Name = "GANKZ_AimLockButton"
    sg.ResetOnSpawn = false
    sg.IgnoreGuiInset = true
    sg.ZIndexBehavior = Enum.ZIndexBehavior.Sibling
...
function GANKZ_ScanMap()
    ...
    GANKZ_Cache.Generators = newGens
    GANKZ_Cache.Gates      = newGates
    GANKZ_Cache.Hooks      = newHooks
    GANKZ_Cache.Pallets    = newPallets
    GANKZ_Cache.Windows    = newWindows
    GANKZ_Cache.ExitPos    = exitPos
    GANKZ_Cache.ExitPart   = exitPart
    print("[GANKZ ScanMap] Generators:", #newGens, "Gates:", #newGates, "Hooks:", #newHooks, "Windows:", #newWindows)

    local root           = Root
    if root and #GANKZ_Cache.Hooks > 0 then
        local closest, closestDist = nil, math.huge
        for _, hook in ipairs(GANKZ_Cache.Hooks) do
            if hook.part then
        ...
        GANKZ_Cache.ClosestHook = closest
    end
end
...
function CreateRadarGUI()
    ...
    radarGui.Name = "GanKunZHub_RadarGUI"
...
function UpdateRadar()
    ...
    if VD.RADAR_ShowGenerator then
        for _, gen in ipairs(GANKZ_Cache.Generators or {}) do
    ...
    if VD.RADAR_ShowPallet then
        for _, pallet in ipairs(GANKZ_Cache.Pallets or {}) do
    ...
    if VD.RADAR_ShowHook then
        for _, hook in ipairs(GANKZ_Cache.Hooks or {}) do
    ...
    if VD.RADAR_ShowGate then
        for _, gate in ipairs(GANKZ_Cache.Gates or {}) do
    ...
    if VD.RADAR_ShowWindow then
        for _, window in ipairs(GANKZ_Cache.Windows or {}) do
    ...
    if VD.RADAR_ShowZombie then
        if GANKZ_WorldReg and GANKZ_WorldReg.SCPZombie then
            for model, entry in pairs(GANKZ_WorldReg.SCPZombie) do
...
function GANKZ_TeleportToPosition(pos)
...
function GANKZ_TeleportToGenerator(index)
    if not GANKZ_Cache or not GANKZ_Cache.Generators or #GANKZ_Cache.Generators == 0 then print("[GANKZ HUB] Generator tidak ditemukan") return false end

    local sorted = {}
    for _, gen in ipairs(GANKZ_Cache.Generators) do
    ...
    return GANKZ_TeleportToPosition(target.gen.part.Position)
end

function GANKZ_TeleportToGate()
    if not GANKZ_Cache or not GANKZ_Cache.Gates or #GANKZ_Cache.Gates == 0 then print("[GANKZ HUB] Gate tidak ditemukan") return false end
    local closest, closestDist = nil, math.huge
    for _, gate in ipairs(GANKZ_Cache.Gates) do
    ...
    return GANKZ_TeleportToPosition(closest.part.Position)
end

function GANKZ_TeleportToHook()
    if not GANKZ_Cache or not GANKZ_Cache.ClosestHook then print("[GANKZ HUB] Hook tidak ditemukan") return false end
    return GANKZ_TeleportToPosition(GANKZ_Cache.ClosestHook.part.Position)
end
...
function CheckMapChange()
    ...
    if CurrentMapName ~= mapName then
        -- print("[GANKZ HUB] Map berubah: " .. tostring(CurrentMapName) .. " -> " .. mapName)
        ...
        if descendant:IsA("Model") or descendant:IsA("Folder") then
            ...
            if n:find("generator") or n:find("mesin") or n:find("pallet") or n:find("window") or n:find("hook") or n:find("gate") then
                task.delay(0.5, function()
                    ...
                    if part then
                        if n:find("generator") or n:find("mesin") then table.insert(GANKZ_Cache.Generators, {model=descendant, part=part})
                        elseif n:find("pallet") then table.insert(GANKZ_Cache.Pallets, {model=descendant, part=part})
                        elseif n:find("window") then table.insert(GANKZ_Cache.Windows, {model=descendant, part=part})
                        elseif n:find("hook") then table.insert(GANKZ_Cache.Hooks, {model=descendant, part=part})
                        elseif n:find("gate") then table.insert(GANKZ_Cache.Gates, {model=descendant, part=part})
                        end
                    end
                end)
            end
...
Workspace.ChildAdded:Connect(function(child)
    ...
    if child and child.Name == "Spearprojectile" and VD.SURV_AutoDodgeSpear and GetRole() == "Survivor" then
        task.spawn(function()
            if getgenv().GANKZ_IsDodging then return end
...
local function GANKZ_AutoAttack()
...
RunService.Heartbeat:Connect(function()
    ...
    pcall(function()
        ...
        local windowGroups = {}
        for _, win in ipairs(GANKZ_Cache.Windows or {}) do
...
RunService.Heartbeat:Connect(function()
    ...
    pcall(function()
        ...
        -- Scan pallet dari GANKZ_Cache + langsung dari workspace
        ...
        -- Iterasi GANKZ_Cache.Pallets untuk cari Palletwrong terdekat
        for _, pal in ipairs(GANKZ_Cache.Pallets or {}) do
...
local function GANKZ_UpdateHitboxes()
...
local function GANKZ_DestroyAllPallets()
...
-- =====================================================
getgenv().GANKZ_IsBreakingGenerator = false

function GANKZ_AutoBreakGene()
    if not VD.KILLER_AutoBreakGene or GetRole() ~= "Killer" then return end
    if getgenv().GANKZ_IsBreakingGenerator then return end
    ...
    if nearest then
        getgenv().GANKZ_IsBreakingGenerator = true
        task.spawn(function()
            pcall(function()
                local r = ReplicatedStorage:FindFirstChild("Remotes")
...
-- =====================================================
getgenv().GANKZ_LastVaultBlockTime = 0

function GANKZ_BlockAllVaults()
    if not VD.KILLER_BlockVaults or GetRole() ~= "Killer" then return end
    
    local now = tick()
    if now - getgenv().GANKZ_LastVaultBlockTime < 1.5 then return end
    getgenv().GANKZ_LastVaultBlockTime = now
    ...
    pcall(function()
        ...
        if vaultsFolder then
            ...
            -- Fallback using GANKZ_Cache.Windows if Map structure is not default
            for _, win in ipairs(GANKZ_Cache.Windows or {}) do
...
-- =====================================================
getgenv().GANKZ_LastPalletBlockTime = 0

function GANKZ_BlockAllPalletDrops()
    if not VD.KILLER_BlockPallets or GetRole() ~= "Killer" then return end

    local now = tick()
    if now - getgenv().GANKZ_LastPalletBlockTime < 2 then return end
    getgenv().GANKZ_LastPalletBlockTime = now
    ...
    pcall(function()
        ...
        -- Fallback: gunakan GANKZ_Cache.Pallets
        for _, pal in ipairs(GANKZ_Cache.Pallets or {}) do
...
-- =====================================================
getgenv().GANKZ_LastPalletBlockDropTime = 0
getgenv().GANKZ_IsBlockingPallets = false

-- Helper: force bersihkan semua attribute yang bisa bikin stuck
local function GANKZ_ForceUnstuck(char)
    pcall(function()
...
function GANKZ_BlockPalletDrop()
    if not VD.KILLER_BlockPalletDrop or GetRole() ~= "Killer" then return end
    if getgenv().GANKZ_IsBlockingPallets then return end

    local now = tick()
    if now - getgenv().GANKZ_LastPalletBlockDropTime < 4 then return end
    getgenv().GANKZ_LastPalletBlockDropTime = now
    ...
    pcall(function()
        ...
        getgenv().GANKZ_IsBlockingPallets = true
...
-- =====================================================
getgenv().GANKZ_AbyssCooldownBypassConnection = nil
getgenv().GANKZ_CorruptHandlerFunc = nil

function GANKZ_StartAbyssCooldownBypass()
    if not getgenv().GANKZ_CorruptHandlerFunc then
        for _, v in pairs(getgc(true)) do
            if type(v) == "function" and islclosure(v) then
                local constants = debug.getconstants(v)
                if table.find(constants, "corrupt") and table.find(constants, "Immobile") then
                    getgenv().GANKZ_CorruptHandlerFunc = v
                    break
                end
            end
        end
    ...
    if getgenv().GANKZ_AbyssCooldownBypassConnection then 
        getgenv().GANKZ_AbyssCooldownBypassConnection:Disconnect() 
    end
    
    getgenv().GANKZ_AbyssCooldownBypassConnection = RunService.Heartbeat:Connect(function()
        if not VD.KILLER_BypassCooldown then return end
        if getgenv().GANKZ_CorruptHandlerFunc then
            local upvalues = debug.getupvalues(getgenv().GANKZ_CorruptHandlerFunc)
            for idx, val in pairs(upvalues) do
                if type(val) == "boolean" then
                    if val == false then
                        debug.setupvalue(getgenv().GANKZ_CorruptHandlerFunc, idx, true)
                    end
                end
            end
        end
...
function GANKZ_StopAbyssCooldownBypass()
    if getgenv().GANKZ_AbyssCooldownBypassConnection then
        getgenv().GANKZ_AbyssCooldownBypassConnection:Disconnect()
        getgenv().GANKZ_AbyssCooldownBypassConnection = nil
    end
...
-- =====================================================
getgenv().GANKZ_JeffCooldownBypassThread = nil

function GANKZ_StartJeffCooldownBypass()
    if getgenv().GANKZ_JeffCooldownBypassThread then return end
    getgenv().GANKZ_JeffCooldownBypassThread = task.spawn(function()
...
function GANKZ_StopJeffCooldownBypass()
    pcall(function()
        local player = game:GetService("Players").LocalPlayer
        local char = player.Character
        if char and char:GetAttribute("Frenzy") == true then
...
getgenv().GANKZ_SlasherCooldownBypassThread = nil

function GANKZ_StartSlasherCooldownBypass()
    if getgenv().GANKZ_SlasherCooldownBypassThread then return end
    ...
    getgenv().GANKZ_SlasherCooldownBypassThread = task.spawn(function()
...
function GANKZ_StopSlasherCooldownBypass()
...
-- ==============================================
getgenv().GANKZ_FakeAttackThread = nil
function GANKZ_ToggleFakeAttack(enabled)
    if not enabled then
        if getgenv().GANKZ_FakeAttackThread then
            task.cancel(getgenv().GANKZ_FakeAttackThread)
            getgenv().GANKZ_FakeAttackThread = nil
        end
        return
    end
    
    if getgenv().GANKZ_FakeAttackThread then return end
    
    getgenv().GANKZ_FakeAttackThread = task.spawn(function()
        local Players = game:GetService("Players")
        local LocalPlayer = Players.LocalPlayer
...
function GANKZ_UpdateCureFlaskLaser()
    ...
    if originPos and targetPos and actionActive then
        if not getgenv().GANKZ_CureFlaskLaserPart then
            local laser = Instance.new("Part")
            ...
            laser.Transparency = 0
            laser.Parent = workspace
            getgenv().GANKZ_CureFlaskLaserPart = laser
        end
        
        local dist = (targetPos - originPos).Magnitude
        if dist > 0.1 then
            local laser = getgenv().GANKZ_CureFlaskLaserPart
            laser.Size = Vector3.new(0.16, 0.16, dist)
            laser.CFrame = CFrame.new((originPos + targetPos) / 2, targetPos)
            laser.Transparency = 0
        end
        ...
        if getgenv().GANKZ_CureFlaskLaserPart then
            getgenv().GANKZ_CureFlaskLaserPart.Transparency = 1
        end
    end
end

function GANKZ_StartCureFlaskLaser()
    if getgenv().GANKZ_CureFlaskLaserThread then return end
    getgenv().GANKZ_CureFlaskLaserThread = game:GetService("RunService").RenderStepped:Connect(function()
        ...
        pcall(GANKZ_UpdateCureFlaskLaser)
    end)
end
...
-- =====================================================
getgenv().GANKZ_HiddenLeapBypassThread = nil

function GANKZ_StartHiddenCooldownBypass()
    if getgenv().GANKZ_HiddenLeapBypassThread then return end
    getgenv().GANKZ_HiddenLeapBypassThread = task.spawn(function()
...
function GANKZ_StopHiddenCooldownBypass()
...
-- =====================================================
local function GANKZ_BeatGameSurvivor()
    ...
    if not exitPos and map then
        pcall(function()
            ...
            if GANKZ_Cache and GANKZ_Cache.ExitPos then
                exitPos = GANKZ_Cache.ExitPos
                finishPart = GANKZ_Cache.ExitPart
                return
            end
        end)
    end
...
-- =====================================================
local function GANKZ_BeatGameKiller()
...
local function GANKZ_AutoHook()
    ...
    if isCarrying then
        ...
        for _, h in ipairs(GANKZ_Cache.Hooks or {}) do
            if h.part then
        ...
        if player ~= LocalPlayer and IsSurvivor(player) and player.Character then
            ...
            if tr and hum then
                ...
                if pct <= 0.25 and pct > 0 then
                    ...
                    if GANKZ_Cache and GANKZ_Cache.Hooks then
                        for _, hh in ipairs(GANKZ_Cache.Hooks) do
...
task.spawn(function()
    while not VD.Destroyed do
        if Root and GANKZ_Cache.Hooks and #GANKZ_Cache.Hooks > 0 then
            local closest, closestDist = nil, math.huge
            for _, hook in ipairs(GANKZ_Cache.Hooks) do
                if hook.part then
                    local d = (hook.part.Position - Root.Position).Magnitude
...
task.spawn(function()
    while not VD.Destroyed do
        pcall(GANKZ_AutoAttack)
        pcall(GANKZ_UpdateHitboxes)
        pcall(GANKZ_DestroyAllPallets)
        pcall(GANKZ_AutoBreakGene)
        pcall(GANKZ_BlockAllVaults)
        pcall(GANKZ_BlockAllPalletDrops)
        pcall(GANKZ_BlockPalletDrop)

        pcall(GANKZ_BeatGameSurvivor)
        pcall(GANKZ_BeatGameKiller)
        pcall(GANKZ_AutoHook)
        task.wait(0.12)
    end
end)
...
local function GANKZ_IsTouchOnAttackButton(input)
    local pos = input.Position
    local playerGui = LocalPlayer:FindFirstChild("PlayerGui")
...
UserInputService.InputBegan:Connect(function(input, gpe)
    ...
    if VD.AIM_Enabled and VD.AIM_UseRMB then
        ...
        elseif input.UserInputType == Enum.UserInputType.Touch and not gpe then
            ...
            if GANKZ_IsTouchOnAttackButton(input) then
                State.AimHolding = true
            end
        end
    end
...
do -- Teleport Tab
    ...
    tpMapSection:AddButton({ Name = "TP to Gen", Callback = function() pcall(function() GANKZ_TeleportToGenerator(1) end) end })
    tpMapSection:AddButton({ Name = "TP to Gate", Callback = function() pcall(GANKZ_TeleportToGate) end })
    tpMapSection:AddButton({ Name = "TP to Hook", Callback = function() pcall(GANKZ_TeleportToHook) end })
end
...
-- =====================================================
-- GANKZT KILLER DISPLAY
-- =====================================================
getgenv().GANKZ_GanKunZKillerGui = nil
getgenv().GANKZ_GanKunZKillerRunning = false

function SetupGanKunZKillerIndicator()
    if getgenv().GANKZ_GanKunZKillerGui then pcall(function() getgenv().GANKZ_GanKunZKillerGui:Destroy() end) end
    getgenv().GANKZ_GanKunZKillerGui = nil
    GANKZ_SetMainInfoPanelText("GanKunZKiller", "GanKunZ Killer Display", "Waiting...")
end

function StartGanKunZKiller()
    if getgenv().GANKZ_GanKunZKillerRunning then return end
    getgenv().GANKZ_GanKunZKillerRunning = true
    SetupGanKunZKillerIndicator()
    task.spawn(function()
        local _genv = getgenv()
        while _genv.VD and _genv.VD.VIS_GanKunZKiller and _genv.GANKZ_GanKunZKillerRunning do
            ...
            if nk then
                GANKZ_SetMainInfoPanelText("GanKunZKiller", "GanKunZ Killer Display", "GanKunZ Killer: " .. (nk == LocalPlayer and "YOU" or nk.DisplayName or nk.Name))
            else
                GANKZ_SetMainInfoPanelText("GanKunZKiller", "GanKunZ Killer Display", "GanKunZ Killer: None")
            end
            task.wait(2)
        end
    end)
...
function StopGanKunZKiller()
    getgenv().GANKZ_GanKunZKillerRunning = false
    if getgenv().GANKZ_GanKunZKillerGui then
        pcall(function() getgenv().GANKZ_GanKunZKillerGui:Destroy() end)
        getgenv().GANKZ_GanKunZKillerGui = nil
    end
    GANKZ_SetMainInfoPanelText("GanKunZKiller", "GanKunZ Killer Display", "Off")
end

-- =====================================================
-- SPECTATOR COUNTER
-- =====================================================
if getgenv().GANKZ_SpectatorCounterGui then
    pcall(function() getgenv().GANKZ_SpectatorCounterGui:Destroy() end)
end
getgenv().GANKZ_SpectatorCounterGui = nil
getgenv().GANKZ_SpectatorCounterRunning = false

function SetupSpectatorCounter()
    if getgenv().GANKZ_SpectatorCounterGui then pcall(function() getgenv().GANKZ_SpectatorCounterGui:Destroy() end) end

    local sg = Instance.new("ScreenGui")
    sg.Name = "GANKZ_SpectatorCounterGui"
    sg.ResetOnSpawn = false
    ...
    local pg = GetSafeGuiParent()
    if pg then
        local oldGui = pg:FindFirstChild("GANKZ_SpectatorCounterGui")
    ...
    getgenv().GANKZ_SpectatorCounterGui = sg
end

function StartSpectatorCounter()
    if getgenv().GANKZ_SpectatorCounterRunning then return end
    getgenv().GANKZ_SpectatorCounterRunning = true
    SetupSpectatorCounter()

    task.spawn(function()
        local _genv = getgenv()
        while _genv.VD and _genv.VD.VIS_SpectatorCounter and _genv.GANKZ_SpectatorCounterRunning and getgenv().GANKZ_SpectatorCounterGui do
            local sg = getgenv().GANKZ_SpectatorCounterGui
...
if getgenv().GANKZ_KillerPerksGui then
    pcall(function() getgenv().GANKZ_KillerPerksGui:Destroy() end)
end
getgenv().GANKZ_KillerPerksGui = nil

local GANKZ_KillerPerksCollapsed = false
local GANKZ_KillerPerksLocked = false
getgenv().GANKZ_KillerPerksRunning = false

local GANKZ_KillerPerkNames = {
    GanKunZInLine = "GanKunZ in Line",
    ["GanKunZ in Line"] = "GanKunZ in Line",
...
function GANKZ_EscapeRichText(text)
    text = tostring(text or "")
    text = text:gsub("&", "&amp;")
    text = text:gsub("<", "&lt;")
    text = text:gsub(">", "&gt;")
...
local function GANKZ_FormatPerkName(name)
    name = tostring(name or "")
    if GANKZ_KillerPerkNames[name] then return GANKZ_KillerPerkNames[name] end
...
local function GANKZ_GetKillerPlayer()
    for _, player in ipairs(Players:GetPlayers()) do
...
local function GANKZ_AddWorkspacePerk(result, seen, rawName, displayName, level)
    ...
    table.insert(result, {
        Raw = rawName,
        Name = displayName and tostring(displayName) or GANKZ_FormatPerkName(rawName),
        Level = level and tostring(level) or nil,
    })
end

local function GANKZ_IsPerkContainer(inst)
...
local function GANKZ_ParseWorkspacePerkName(name)
    name = tostring(name or "")
...
local function GANKZ_ReadPerksFromWorkspace(killer)
        ...
        for key, value in pairs(attrs) do
            ...
            if lowerKey:find("perk") then
                if type(value) == "string" then
                    GANKZ_AddWorkspacePerk(result, seen, value)
                elseif value == true then
                    GANKZ_AddWorkspacePerk(result, seen, key)
                elseif type(value) == "number" and lowerKey:find("level") then
                    local baseName = tostring(key):gsub("[Ll]evel", ""):gsub("[Pp]erk", "")
                    if baseName ~= "" then
                        GANKZ_AddWorkspacePerk(result, seen, baseName, nil, value)
                    end
                end
            end
        end
    ...
    for _, child in ipairs(char:GetChildren()) do
        local perkName, level = GANKZ_ParseWorkspacePerkName(child.Name)
        if perkName then
            GANKZ_AddWorkspacePerk(result, seen, child.Name, perkName, level)
        end
    end
    ...
    for _, inst in ipairs(char:GetDescendants()) do
        ...
        if GANKZ_IsPerkContainer(inst) then
            for _, child in ipairs(inst:GetChildren()) do
                local value, level = readValueObject(child)
                GANKZ_AddWorkspacePerk(result, seen, value or child.Name, nil, level)
            end
        else
            local lowerName = inst.Name:lower()
            if lowerName:find("perk") then
                local value, level = readValueObject(inst)
                GANKZ_AddWorkspacePerk(result, seen, value or inst.Name, nil, level)
            end
        end
    end
...
local function GANKZ_BuildKillerPerksText()
    local killer = GANKZ_GetKillerPlayer()
    local killerName = killer and (killer.DisplayName or killer.Name) or "Unknown"
    local perks = GANKZ_ReadPerksFromWorkspace(killer)

    if #perks == 0 then
        for _, player in ipairs(Players:GetPlayers()) do
            local candidatePerks = GANKZ_ReadPerksFromWorkspace(player)
    ...
    local lines = {
        'Killer Perks [<font color="rgb(255,80,80)">' .. GANKZ_EscapeRichText(killerName) .. '</font>]',
    }
    ...
    if #perks == 0 then
        ...
        for i = 1, math.min(#perks, 4) do
            ...
            table.insert(lines, '<font color="rgb(255,204,80)">- ' .. GANKZ_EscapeRichText(perk.Name) .. GANKZ_EscapeRichText(levelText) .. '</font>')
        end
    end
...
local function SetupKillerPerksDisplay()
    if getgenv().GANKZ_KillerPerksGui then pcall(function() getgenv().GANKZ_KillerPerksGui:Destroy() end) end
    getgenv().GANKZ_KillerPerksGui = nil
    GANKZ_SetMainInfoPanelText("KillerPerks", "Spectate Killer Perks", "Waiting for perk data...")
end

function StartKillerPerksDisplay()
    if getgenv().GANKZ_KillerPerksRunning then return end
    getgenv().GANKZ_KillerPerksRunning = true
    SetupKillerPerksDisplay()

    task.spawn(function()
        local _genv = getgenv()
        while _genv.VD and _genv.VD.VIS_KillerPerks and _genv.GANKZ_KillerPerksRunning do
            local text = GANKZ_BuildKillerPerksText()
            GANKZ_SetMainInfoPanelText("KillerPerks", "Spectate Killer Perks", text)
            task.wait(1)
        end
    end)
end
...
function StopKillerPerksDisplay()
    ...
    GANKZ_SetMainInfoPanelText("KillerPerks", "Spectate Killer Perks", "Off")
end
...
if getgenv().GANKZ_PredictMapGui then
    pcall(function() getgenv().GANKZ_PredictMapGui:Destroy() end)
end
if getgenv().GANKZ_PredictMapConnections then
    for _, conn in ipairs(getgenv().GANKZ_PredictMapConnections) do
        pcall(function() conn:Disconnect() end)
    end
end
getgenv().GANKZ_PredictMapGui = nil
getgenv().GANKZ_PredictMapConnections = {}
getgenv().GANKZ_PredictMapRunning = false

local GANKZ_PredictMapState = {
    Name = "Unknown",
...
local function GANKZ_MapStringFromValue(value)
    if type(value) == "string" then
        return value
...
local function GANKZ_MapDescFromValue(value)
    if type(value) ~= "table" then return nil end
    return value.Desc or value.desc or value.Description or value.description
end

local function GANKZ_SetPredictedMap(name, desc, source)
    name = GANKZ_MapStringFromValue(name) or name
    if not name or tostring(name) == "" then return end
    GANKZ_PredictMapState.Name = tostring(name)
    if desc and tostring(desc) ~= "" then
        GANKZ_PredictMapState.Desc = tostring(desc)
    end
    GANKZ_PredictMapState.Source = source or "Detected"
end

local function GANKZ_ReadCurrentWorkspaceMap()
    local map = Workspace:FindFirstChild("Map")
...
local function GANKZ_TryPredictMapRemote()
    local remotes = ReplicatedStorage:FindFirstChild("Remotes")
    ...
    for _, args in ipairs(attempts) do
        ...
        if ok and data ~= nil then
            local name = GANKZ_MapStringFromValue(data)
            local desc = GANKZ_MapDescFromValue(data)
            if name then
                GANKZ_SetPredictedMap(name, desc, "Predicted")
                return true
            end
        end
    end
...
local function GANKZ_PredictMapText()
    local name = GANKZ_EscapeRichText(GANKZ_PredictMapState.Name or "Unknown")
    local source = GANKZ_EscapeRichText(GANKZ_PredictMapState.Source or "Idle")
    local phase = tostring(GANKZ_PredictMapState.Phase or "")
    local timeLeft = GANKZ_PredictMapState.TimeLeft
    local timerText = ""
    if timeLeft then
...
local function SetupPredictMapGui()
    if getgenv().GANKZ_PredictMapGui then pcall(function() getgenv().GANKZ_PredictMapGui:Destroy() end) end
    getgenv().GANKZ_PredictMapGui = nil
    GANKZ_SetMainInfoPanelText("PredictMap", "Predict Map", GANKZ_PredictMapText())
end

local function GANKZ_BindPredictMapEvents()
    local conns = getgenv().GANKZ_PredictMapConnections
    local remotes = ReplicatedStorage:FindFirstChild("Remotes")
    ...
    if mapInfo and mapInfo:IsA("RemoteEvent") then
        table.insert(conns, mapInfo.OnClientEvent:Connect(function(a, b, c)
            local name = GANKZ_MapStringFromValue(a) or GANKZ_MapStringFromValue(b) or GANKZ_MapStringFromValue(c)
            local desc = GANKZ_MapDescFromValue(a) or GANKZ_MapDescFromValue(b) or GANKZ_MapDescFromValue(c)
            if name then
                GANKZ_SetPredictedMap(name, desc, "Predicted")
            end
        end))
    ...
    if timeEvent and timeEvent:IsA("RemoteEvent") then
        table.insert(conns, timeEvent.OnClientEvent:Connect(function(phase, timeLeft)
            GANKZ_PredictMapState.Phase = tostring(phase or "")
            GANKZ_PredictMapState.TimeLeft = tonumber(timeLeft)
            if tostring(phase) == "Intermission" and tonumber(timeLeft) and tonumber(timeLeft) <= 20 then
                pcall(GANKZ_TryPredictMapRemote)
            end
        end))
    end
    ...
    table.insert(conns, Workspace.ChildAdded:Connect(function(child)
        if child and child.Name == "Map" then
            task.delay(0.25, function()
                local name, desc = GANKZ_ReadCurrentWorkspaceMap()
                if name then GANKZ_SetPredictedMap(name, desc, "Confirmed") end
            end)
        end
    end))
end
...
function StartPredictMap()
    if getgenv().GANKZ_PredictMapRunning then return end
    getgenv().GANKZ_PredictMapRunning = true
    SetupPredictMapGui()
    GANKZ_BindPredictMapEvents()

    task.spawn(function()
        while VD and VD.VIS_PredictMap and getgenv().GANKZ_PredictMapRunning do
            local currentName, currentDesc = GANKZ_ReadCurrentWorkspaceMap()
            if currentName then
                GANKZ_SetPredictedMap(currentName, currentDesc, "Confirmed")
            else
                pcall(GANKZ_TryPredictMapRemote)
            end
...
function StopPredictMap()
    ...
    getgenv().GANKZ_PredictMapConnections = {}
    GANKZ_SetMainInfoPanelText("PredictMap", "Predict Map", "Off")
end
...
-- =====================================================
getgenv().GANKZ_OriginalFOV          = nil
getgenv().GANKZ_OriginalCameraType   = nil
getgenv().GANKZ_OriginalCameraOffset = nil
getgenv().GANKZ_ThirdPersonWasActive = false
getgenv().GANKZ_FOVWasActive         = false
...
function UpdateCameraFOV()
    ...
    if VD.CAM_FOVEnabled then
        if not getgenv().GANKZ_FOVWasActive then
            getgenv().GANKZ_OriginalFOV = cam.FieldOfView
            getgenv().GANKZ_FOVWasActive = true
        end
        cam.FieldOfView = VD.CAM_FOV or 90
    elseif getgenv().GANKZ_FOVWasActive then
        if getgenv().GANKZ_OriginalFOV then cam.FieldOfView = getgenv().GANKZ_OriginalFOV end
        getgenv().GANKZ_OriginalFOV = nil
        getgenv().GANKZ_FOVWasActive = false
    end
end
...
function UpdateThirdPerson()
    ...
    if shouldBeActive then
        if not getgenv().GANKZ_ThirdPersonWasActive then
            getgenv().GANKZ_OriginalCameraType = cam.CameraType
            local char = LocalPlayer.Character
            local hum = char and char:FindFirstChildOfClass("Humanoid")
            getgenv().GANKZ_OriginalCameraOffset = hum and hum.CameraOffset or Vector3.new(0, 0, 0)
        end
        cam.CameraType = Enum.CameraType.Custom
        local char     = LocalPlayer.Character
        local hum      = char and char:FindFirstChildOfClass("Humanoid")
        if hum then hum.CameraOffset = Vector3.new(2, 1, 8) end
        getgenv().GANKZ_ThirdPersonWasActive = true
    elseif getgenv().GANKZ_ThirdPersonWasActive then
        if getgenv().GANKZ_OriginalCameraType then
            cam.CameraType = getgenv().GANKZ_OriginalCameraType; getgenv().GANKZ_OriginalCameraType = nil
        end
        local char = LocalPlayer.Character
        local hum  = char and char:FindFirstChildOfClass("Humanoid")
        if hum then hum.CameraOffset = getgenv().GANKZ_OriginalCameraOffset or Vector3.new(0, 0, 0) end
        getgenv().GANKZ_OriginalCameraOffset = nil
        getgenv().GANKZ_ThirdPersonWasActive = false
    end
end

getgenv().GANKZ_shiftLockWasActive = false
...
function UpdateShiftLock()
    ...
    if VD.CAM_ShiftLock then
        ...
        getgenv().GANKZ_shiftLockWasActive = true
        ...
        if getgenv().GANKZ_shiftLockWasActive then
        ...
        if map then
            for _, obj in ipairs(map:GetDescendants()) do
                if obj.Name:lower():find("fog") or obj:IsA("Atmosphere") or obj:IsA("BloomEffect") or obj:IsA("BlurEffect") or obj:IsA("ColorCorrectionEffect") then
                    if not getgenv().GANKZ_FogCache[obj] then
                        getgenv().GANKZ_FogCache[obj] = {
    ...
    pcall(function()
        ...
        for _, obj in ipairs(lt:GetChildren()) do
            if obj:IsA("Atmosphere") or obj.Name:lower():find("fog") then
                if not getgenv().GANKZ_FogCache[obj] then getgenv().GANKZ_FogCache[obj] = { enabled = true, parent = obj.Parent } end
...
function RestoreFog()
    pcall(function()
        for obj, data in pairs(getgenv().GANKZ_FogCache) do
...
function SetupAntiStunSlowdown()
    if getgenv().GANKZ_AntiStunHooked then return end
    getgenv().GANKZ_AntiStunHooked = true
...
function SetupNoCutsceneHook()
    if getgenv().GANKZ_NoCutsceneHooked then return end
    getgenv().GANKZ_NoCutsceneHooked = true
...
function GANKZ_SetupNoCutsceneListeners()
    task.spawn(function()
...
task.spawn(GANKZ_SetupNoCutsceneListeners)
...
-- =====================================================
getgenv().GANKZ_MobileGui = getgenv().GANKZ_MobileGui or { AimBtn=nil, FOVFrame=nil, FOVStroke=nil }
...
function CreateMobileUI()
    ...
    fovStk.Parent = fovF
    getgenv().GANKZ_MobileGui.FOVFrame = fovF; getgenv().GANKZ_MobileGui.FOVStroke = fovStk

    -- === AIMBOT BUTTON (ScreenGui terpisah agar AlwaysOnTop) ===
    local aimSG = Instance.new("ScreenGui")
    aimSG.Name           = "GANKZ_AimBtn"
    ...
    getgenv().GANKZ_MobileGui.AimBtn = btn
end
...
function setupSpearAimbotBtn()
    ...
    print("[GanKunZ Hub] Spear Aimbot Button created successfully")
end
...
function UpdateMobileFOV()
    if not getgenv().GANKZ_MobileGui.FOVFrame then return end
    if VD.AIM_Enabled and VD.AIM_ShowFOV then
        local r = (VD.AIM_FOV or 120)
        getgenv().GANKZ_MobileGui.FOVFrame.Size = UDim2.new(0, r*2, 0, r*2)
        getgenv().GANKZ_MobileGui.FOVStroke.Color = State.AimTarget and Color3.fromRGB(90,220,120) or Color3.fromRGB(220,70,70)
        getgenv().GANKZ_MobileGui.FOVFrame.Visible = true
    else
        getgenv().GANKZ_MobileGui.FOVFrame.Visible = false
    end
end
...
RunService.Heartbeat:Connect(function(deltaTime)
    ...
    if not DrawingAvailable and (not getgenv().GANKZ_MobileGui.FOVFrame or not getgenv().GANKZ_MobileGui.FOVFrame.Parent) then
        pcall(CreateMobileUI)
    end
    ...
    if not DrawingAvailable then
        if getgenv().GANKZ_MobileGui.AimBtn then getgenv().GANKZ_MobileGui.AimBtn.Visible = VD.AIM_Enabled end
...
-- =====================================================
getgenv().GANKZ_SyncLoadedFeatures = function()
    ...
    -- Sync GanKunZ Killer Display
    if VD.VIS_GanKunZKiller then
        pcall(StartGanKunZKiller)
    else
        pcall(StopGanKunZKiller)
    end
    ...
    if getgenv().GANKZ_SetHideSurvivorIcon then
        pcall(getgenv().GANKZ_SetHideSurvivorIcon, VD.VIS_HideSurvivorIcon)
    end

    -- Sync Ping & FPS Display
    if getgenv().GANKZ_SetShowPingFPS then
        pcall(getgenv().GANKZ_SetShowPingFPS, VD.VIS_ShowPingFPS)
    end

    -- Sync Hook Counter
    if getgenv().GANKZ_SetShowHookCounter then
        pcall(getgenv().GANKZ_SetShowHookCounter, VD.VIS_ShowHookCounter)
    end

    -- Sync Twist of Fate silent aim
    if getgenv().GANKZ_SetToFSilentAim then
        pcall(getgenv().GANKZ_SetToFSilentAim, VD.TOF_SilentAim)
    end

    -- Sync Flashlight silent aim
    if getgenv().GANKZ_SetFlashlightSilentAim then
        pcall(getgenv().GANKZ_SetFlashlightSilentAim, VD.FLASH_SilentAim)
    end
...
getgenv().GANKZ_SyncUILibraryConfigRuntime = function()
    local tofValue = readConfigElementValue("Silent Aim Twist Of Fate")
    if type(tofValue) == "boolean" and tofValue ~= VD.TOF_SilentAim and getgenv().GANKZ_SetToFSilentAim then
        pcall(getgenv().GANKZ_SetToFSilentAim, tofValue)
    elseif type(tofValue) == "boolean" then
        VD.TOF_SilentAim = tofValue
        if tofValue and getgenv().GANKZ_SetToFSilentAim then
            pcall(getgenv().GANKZ_SetToFSilentAim, true)
        end
    end

    local flashValue = readConfigElementValue("Silent Aim Flashlight")
    if type(flashValue) == "boolean" and flashValue ~= VD.FLASH_SilentAim and getgenv().GANKZ_SetFlashlightSilentAim then
        pcall(getgenv().GANKZ_SetFlashlightSilentAim, flashValue)
    elseif type(flashValue) == "boolean" then
        VD.FLASH_SilentAim = flashValue
        if flashValue and getgenv().GANKZ_SetFlashlightSilentAim then
            pcall(getgenv().GANKZ_SetFlashlightSilentAim, true)
        end
    end
end
